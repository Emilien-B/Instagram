...
class InstaDM(object):
    ...
    def __init__(self, username, password, headless=True, instapy_workspace=None, profileDir=None, waiting=True):
    ...
    def sendMessage(self, user, message, greeting=None, send=True):
        self.send = send
        self.user = user
    ...
    def NewMsg(self, exp=""):
        ...
        if exp == "":
            ...
            try:
                self.TurnOn_btn = self.driver.find_element_by_xpath("//button[text()='Turn On']")
                self.TurnOn_btn[0].click()
                self.NotNow_btn = self.driver.find_element_by_xpath("//button[text()='Not Now']")
                self.NotNow_btn.click()
            except:
                self.value = 0
            self.NewMsgs = self.driver.find_elements_by_xpath("//*[@class='_7UhW9   xLCgt       qyrsm KV-D4           se6yk        ']")
            self.NewMsgs_exp = self.driver.find_elements_by_xpath("//*[@class='_7UhW9   xLCgt       qyrsm KV-D4              fDxYl     ']")
            self.résultat = {}
            self.value = 0
            for msg in self.NewMsgs:
                self.résultat.update({self.NewMsgs_exp[self.value].text:self.NewMsgs[self.value].text})
                self.value = self.value + 1
            print("Recovered data")
            return self.résultat
        else:
            self.sendMessage(user=exp, message="Z", send=False)
            self.next_btn = self.driver.find_element_by_xpath("//*[@class='rIacr']")
            self.next_btn.click()
            self.msgs = self.driver.find_elements_by_xpath("//*[@class='_7UhW9   xLCgt      MMzan  KV-D4             p1tLr      hjZTB']")
            self.msgs_exp_result = self.driver.find_elements_by_xpath("//*[@class='  _6FEQj CMoMH    RQUXn _8_yLp  ']")
            self.msgs_exp = []
            for msg in self.msgs_exp_result:
                self.msgs_exp.append(msg.text)
            self.résultat = []
            self.résultat_exp = []

            for msg in self.msgs:
                self.résultat.append(msg.text)
                if self.msgs_exp.count(msg.text) >= 1:
                    self.résultat_exp.append(True)
                else:
                    self.résultat_exp.append(False)
            print("Recovered data")
            return self.résultat, self.résultat_exp
            
            
    def sendGroupIDMessage(self, chatID, message, send=True):
        self.send = send
